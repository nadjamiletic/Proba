...
<ul id="menu">
	...
	<div class="demo-info" style="margin-bottom:10px">
		...
		<center><h2>UNOS REDA VOZNJE</h2></center>
	...
	<table id="tt" class="easyui-datagrid" style="width:1000px;height:680px"
		...
		<thead>
			<tr>
			    ...
			    <th field="imes" width="0" sortable="true">Stanica</th>
			    <th field="brojpolaska" width="0" sortable="true">Broj Polaska</th>
				<th field="vreme" width="0" sortable="true">Vreme Polaska</th>
				<th field="dani" width="0" sortable="true">Dani Polaska</th>
				<th field="Km" width="0" sortable="true">KM</th>
	...
	<div id="dlg" class="easyui-dialog" style="width:400px;height:580px;padding:10px 20px"
		...
		<form id="fm" method="post" novalidate>
			...
			<div class="fitem">
				...
				$stmt = $dbh->prepare('select linijaid, imel from linija order by linijaid');
				...
				$stmt = $dbh->prepare('select stanicaid, imes from stanica order by stanicaid');
                $stmt->setFetchMode(PDO::FETCH_ASSOC);
                $stmt->execute();
                $data = $stmt->fetchAll();
				...
				<label>Stanica:</label>
				<select class="easyui-combobox" required="true" name="stanicaid" style="width:200px;">
				
                <?php foreach ($data as $row): ?>
				<option><?=$row["stanicaid"]?>.<?=$row["imes"]?></option>
			...
			<div class="fitem">
				<label> Broj Polaska</label>
				<input name="brojpolaska" class="easyui-textbox" required="true">
			</div>
			<div class="fitem">
				<label> Vreme</label>
				<input name="vreme" class="easyui-textbox" required="true">
			</div>
			<div class="fitem">
				<label> Dani</label>
				<input name="dani" class="easyui-textbox" required="true">
			</div>
			<div class="fitem">
				<label> KM</label>
				<input name="Km" class="easyui-textbox" required="true">
			</div>
	...
	<script type="text/javascript">
		...
		function editUser(){
			...
			if (row){
				...
				url = 'update_user.php?redvoznjeid='+row.redvoznjeid;
			}
		}
		function saveUser(){
